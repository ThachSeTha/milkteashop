...
<body>
    <div class="form-container">
        <h1><i class="fas fa-user-edit icon"></i> Chỉnh sửa nhân viên</h1>
        ...
        <form action="{{ route('nhanviens.update', $nhanvien->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" id="ho_ten" name="ho_ten" value="{{ $nhanvien->ho_ten }}" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" id="so_dien_thoai" name="so_dien_thoai" value="{{ $nhanvien->so_dien_thoai }}" required>
            </div>
            <div class="form-group">
                <label for="chuc_vu"><i class="fas fa-briefcase icon"></i> Chức vụ:</label>
                <select id="chuc_vu" name="chuc_vu">
                    @foreach ($chucVus as $chucVu)
                        <option value="{{ $chucVu->id }}" {{ $nhanvien->chuc_vu == $chucVu->id ? 'selected' : '' }}>{{ $chucVu->ten_chuc_vu }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label for="dia_chi"><i class="fas fa-map-marker-alt icon"></i> Địa chỉ:</label>
                <input type="text" id="dia_chi" name="dia_chi" value="{{ $nhanvien->dia_chi }}" required>
            </div>
            <button type="submit"><i class="fas fa-save icon"></i> Cập nhật</button>
        </form>
